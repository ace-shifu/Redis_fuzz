...
static REDIS_STATUS execute_result;
#endif
...
public:
    ...
    bool connect()
    {
        ...
        {
            fprintf(stderr, "Can 's allocate redis context \n");
            return false;
        }
        else
        {
    ...
    REDIS_STATUS execute(char *cmd)
    {
        auto conn = connect();
        auto serverStatus=rNormal;
        ...
        redisReply* reply = (redisReply*)redisCommand(r,(const char*) cmd);
	ACTF("Redis exuces success");
...
RedisClient g_myredisclient((char*)"127.0.0.1",6379);
...
BEGIN:
  ...
  g_current_input="set aa 1";
  auto result = g_myredisclient.execute(g_current_input);

#ifdef COUNT_ERROR
  execute_result = result;
#endif

  if(result == rServerCrash){
    status = FAULT_CRASH;
  }else if(result == rTimeout){
    status = FAULT_TMOUT;
  }else if( result == rConnectFailed){
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "REDISFuzzer " cBRI VERSION cRST " by hackers\n");//string_lib he common_string_lib YOUSHENME QUBIE
